...
import { ScreenSection } from "./apps/frontend/src/components/ui/Screen";
import type { Paciente, Tratamento, LinkCompartilhamento } from "./apps/frontend/src/types/app";
import { useHomeResumo, useLinksCompartilhamento } from "./apps/frontend/src/hooks/useDashboard";
import { useDosesProximas, useConfirmarDose } from "./apps/frontend/src/hooks/useDoses";
import { useTratamentos } from "./apps/frontend/src/hooks/useTratamentos";
import { usePacientes } from "./apps/frontend/src/hooks/usePacientes";
import { useToast } from "./apps/frontend/src/components/Toast";
import { precacheProximasDoses } from "./apps/frontend/src/serviceWorkerRegistration";
import { listarDosesProximas } from "./apps/frontend/src/services/api";
...
export default function HomeScreen() {
  ...
  const pacientesMap = useMemo(
    () => new Map<string, string>(pacientes.map((paciente) => [paciente.id, paciente.nome] as const)),
    [pacientes]
  );
  const tratamentosMap = useMemo(
    () => new Map<string, Tratamento>(tratamentos.map((tratamento) => [tratamento.id, tratamento] as const)),
    [tratamentos]
  );
    ...
    () => [
      {
        ...
        to: "/app/familia",
        icon: FamilyIcon,
        accent: "from-sky-200/40 via-sky-100/40 to-white",
      ...
      },
      {
        ...
        to: "/app/tratamentos",
        icon: PillAppIcon,
        accent: "from-violet-200/40 via-violet-100/40 to-white",
      ...
      },
      {
        ...
        accent: "from-amber-200/40 via-amber-100/40 to-white",
      ...
      },
      {
        ...
        to: "/app",
        icon: HomeAppIcon,
        accent: "from-emerald-200/40 via-emerald-100/40 to-white",
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ScreenSection
        title="Compartilhamentos rápidos"
      ...
      >
        {linksLoading ? (
          ...
          <ul className="flex flex-col gap-3">
            {links.slice(0, 4).map((link: LinkCompartilhamento) => (
...
function MobileHero({ resumo, navigate }: MobileHeroProps) {
  const stats = [
    { label: "Fam�lias", value: numberFormatter.format(resumo?.pacientes ?? 0) },
  ...
  return (
    <div className="rounded-[34px] border border-border/50 bg-gradient-to-br from-primary/12 via-white to-white px-5 py-6 shadow-soft">
      <div className="flex items-center gap-4">
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white text-primary shadow-inner">
      ...
      <div className="mt-4 flex items-center gap-2 text-xs text-muted">
        {stats.map((stat) => (
          <span key={stat.label} className="flex-1 rounded-xl bg-white/70 px-3 py-2 text-center font-semibold text-primary shadow-inner">
            {stat.value}
...
function AppModuleCard({ module, onNavigate }: AppModuleCardProps) {
  ...
  return (
    <div
      ...
      className={`group relative flex flex-col gap-3 overflow-hidden rounded-3xl border border-border/40 bg-gradient-to-br ${module.accent} p-4 text-left shadow-soft transition-transform duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 active:scale-[0.99] hover:-translate-y-1 hover:shadow-elevated`}
    >
      <div className="flex items-start justify-between gap-2">
        <span className="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-white/85 text-primary shadow-inner">
          <module.icon className="h-7 w-7" />
        </span>
        {module.badge && (
          <span className="rounded-full bg-white/75 px-3 py-1 text-[11px] font-semibold text-muted shadow-soft">
            {module.badge}
          </span>
        )}
      </div>
      ...
      {module.quickAction && (
        <div className="flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-xl border border-white/80 bg-white/80 px-3 py-1 text-[11px] font-semibold text-primary shadow-soft transition hover:bg-white"
          >
